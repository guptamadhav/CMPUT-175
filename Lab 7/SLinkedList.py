...
class SLinkedList:
    ...
    def insert(self,pos,item):
        ...
        if pos == 0:
            ...
            new_node.setNext(current.getNext())#here the current node is pushed one position to the right of new node
            current.setNext(new_node)# and the new node is set at the position which is one position left to the current node
                                     # since current iterates upto pos-1
            self.size += 1
    ...
    def remove(self,item):
        ...
        while current != None and not found:
            if current.getData() == item:
                ...
                previous = current # to keep a track of the previous node
                current = current.getNext()
        if not found:
